...
class GrabbingTaxi:
    def minTime(self, tXs, tYs, gX, gY, walkTime, taxiTime):
    	minimum_time_consume = None
        for x, y in zip(tXs, tYs):
        	walk_to_taxi_time = (abs(x) + abs(y)) * walkTime
        	taxi_to_office_time = (abs(x - gX) + abs(y - gY)) * taxiTime
        	total_time = walk_to_taxi_time + taxi_to_office_time
        	if minimum_time_consume == None or minimum_time_consume > total_time:
        		minimum_time_consume = total_time
        		
        walk_to_office_time = (abs(gX) + abs(gY)) * walkTime
        
        if minimum_time_consume is None or walk_to_office_time < minimum_time_consume:
        	return walk_to_office_time
        else:
        	return minimum_time_consume
